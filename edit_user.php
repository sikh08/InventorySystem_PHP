...
<?php
  ...
  if(!$e_user){
    $session->msg("d","Falta id usuario .");
    redirect('users.php');
  }
?>
       ...
       $status   = remove_junk($db->escape($_POST['status']));
         ...
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            ...
            $session->msg('d',' Fallo actualizar!');
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          }
    } else {
...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
       ...
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
          $session->msg('s',"Contraseña usuario ha sido actulizada ");
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        } else {
          $session->msg('d',' Fallo actualizar contraseña!');
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        }
  } else {
    $session->msg("d", $errors);
     ...
     <div class="panel panel-default">
       ...
       <div class="panel-body">
          <form method="post" action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" class="clearfix">
            ...
            <div class="form-group">
              <label for="level">Role usuario</label>
      ...
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" method="post" class="clearfix">
          <div class="form-group">
                ...
                <input type="password" class="form-control" name="password" placeholder="Escribe la contraseña del usuario">
          </div>
          <div class="form-group clearfix">
