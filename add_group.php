...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Lo sentimos!</b> El nombre del grupo ya existe!');
     redirect('add_group.php', false);
   }elseif(find_by_groupLevel($_POST['group-level']) === false) {
     $session->msg('d','<b>Lo sentimos!</b> Nivel del grupo ya existe!');
     redirect('add_group.php', false);
   }
   if(empty($errors)){
        ...
        if($db->query($query)){
          ...
          //failed
          $session->msg('d',' Lo sentimos grupo no creado!');
          redirect('add_group.php', false);
        }
   } else {
...
<div class="login-page">
    <div class="text-center">
       <h3>Añadir nuevo usuario al grupo</h3>
     </div>
