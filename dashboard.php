...
<body>
    <div>
        ...
        $res=mysqli_query($link,"SELECT p.name, COUNT(s.product_id) AS totalSold, SUM(s.qty) AS totalQty FROM sales s LEFT JOIN products p ON p.id = s.product_id GROUP BY s.product_id ORDER BY SUM(s.qty) DESC");//seleccion de tablas
        while($row=mysqli_fetch_array($res)){//bucles para recorrido de datos

            $test[$count]["label"]=$row["name"];
            $test[$count]["y"]=$row["totalQty"];
            $count=$count+1;
    ...
    <div>
        ...
        <body>
        <div id="chartContainer" style="height: 100; width: 10%;"></div>
        <script src="https://canvasjs.com/assets/script/canvasjs.min.js"></script>
        </body>
        </html>                              
    </div>
