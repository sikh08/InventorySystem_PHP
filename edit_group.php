...
<?php
  ...
  if(!$e_group){
    $session->msg("d","Falta el id grupo.");
    redirect('group.php');
  }
?>
<?php
   ...
   if(empty($errors)){
        ...
        $result = $db->query($query);
         if($result && $db->affected_rows() === 1){
          //sucess
          $session->msg('s',"Grupo ha sido actualizado! ");
          redirect('edit_group.php?id='.(int)$e_group['id'], false);
        } else {
          //failed
          $session->msg('d',' Fallo al actualizar grupo!');
          redirect('edit_group.php?id='.(int)$e_group['id'], false);
        }
   } else {
    ...
    <div class="text-center">
     ...
     <?php echo display_msg($msg); ?>
      <form method="post" action="edit_group.php?id=<?php echo (int)$e_group['id'];?>" class="clearfix">
        ...
        <div class="form-group">
              <label for="level" class="control-label">Nivel grupo</label>
